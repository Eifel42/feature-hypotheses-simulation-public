...
validate_sonar_reports() {
    ...
    python3 - "$ROOT_DIR" "$coverage_report" "$junit_report" <<'PY'
import sys
import xml.etree.ElementTree as ET
from pathlib import Path
...
if line_rate < 0.80:
    raise SystemExit(f"Line coverage gate failed: {line_rate:.1%} < 80.0%")
if branch_rate < 0.80:
    raise SystemExit(f"Condition coverage gate failed: {branch_rate:.1%} < 80.0%")
...
run_sonar_scan() {
    ...
    log_info "Running SonarScanner against external ${SONAR_HOST_URL} ..."
...
case $COMMAND in
    ...
    sbom)
        ...
        docker_compose --profile dev run --rm fhs-dev bash -c "
            ...
            pip show cyclonedx-bom >/dev/null 2>&1 || pip install --no-cache-dir cyclonedx-bom &&
