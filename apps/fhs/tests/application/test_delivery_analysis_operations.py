...
class TestSprintPlanRows:
    def test_uses_configured_sprint_length(self) -> None:
        feature = _feature("H9: Partial Sprint", 12_000.0, weeks=5.0)
        config = DeliveryRiskConfig().model_copy(update={"sprint_length_weeks": 1})

        rows = DeliveryAnalysisOperations.sprint_plan_rows([feature], config)

        assert rows == [("Partial Sprint", "5.0", "5", "EUR 12,000")]
...
# ---------------------------------------------------------------------------
# Edge cases for missing coverage branches
# ---------------------------------------------------------------------------


class TestCostComparisonRowsEdgeCases:
    def test_skips_feature_without_delivery_result(self) -> None:
        features = [_feature("H1: Alpha", 40_000), _feature("H2: Beta", 60_000)]
        results = {
            "H1: Alpha": _delivery_result("H1: Alpha", 4, [40_000, 44_000]),
        }
        rows = DeliveryAnalysisOperations.cost_comparison_rows(features, results)
        assert len(rows) == 1
        assert "Alpha" in rows[0][0]


class TestCostRiskCombinationRowsEdgeCases:
    def test_returns_empty_when_no_features_have_delivery_results(self) -> None:
        features = [_feature("H1: Alpha", 40_000)]
        rows = DeliveryAnalysisOperations.cost_risk_combination_rows(
            features, delivery_results={}, budget=100_000.0
        )
        assert rows == []
