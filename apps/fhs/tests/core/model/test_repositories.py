...
class TestYamlFeatureRepository:
    ...
    def test_get_scenario_loads_deployment_risk_block(
        self, temp_config_dir, sample_scenario_config
    ):
        """Repository should parse optional deployment_risk block into ScenarioConfig."""
        repo = YamlFeatureRepository(config_dir=temp_config_dir)
        config = copy.deepcopy(sample_scenario_config)
        config["deployment_risk"] = {
    ...
    def test_get_scenario_without_deployment_risk_defaults_to_none(
        self, temp_config_dir, sample_scenario_config
    ):
        """Repository should accept configs that omit optional deployment_risk block."""
        repo = YamlFeatureRepository(config_dir=temp_config_dir)
        config = copy.deepcopy(sample_scenario_config)
        config.pop("deployment_risk", None)
