...
class TestScenarioConfig:
    ...
    def test_deployment_and_delivery_risk_keys_are_mutually_exclusive(self):
        """Scenario config should reject both sprint-risk keys in one file."""
        with pytest.raises(ValidationError, match="Use either deployment_risk"):
            ScenarioConfig(
                scenario_id="demo",
                name="Demo",
                deployment_risk={"sprint_length_weeks": 2},
                delivery_risk={"sprint_length_weeks": 2},
                features=[
                    Feature(
                        name="F1",
                        expected_users=1000,
