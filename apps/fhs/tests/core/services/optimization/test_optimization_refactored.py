...
def test_objective_function_supported_strategies_returns_all_known() -> None:
    from fhs.core.services.optimization.scoring.objective_function import (
        ObjectiveFunctionFactory,
    )

    strategies = ObjectiveFunctionFactory.supported_strategies()
    assert isinstance(strategies, tuple)
    assert "var_floor" in strategies
