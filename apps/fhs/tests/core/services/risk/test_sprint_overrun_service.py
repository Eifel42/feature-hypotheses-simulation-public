...
class TestComputeSprintPlans:
    ...
    def test_planned_sprints_helper_uses_configured_sprint_length(self):
        assert (
            SprintOverrunService.planned_sprints_for_weeks(
                5,
                sprint_length_weeks=1.0,
            )
            == 5
        )

    def test_compute_sprint_plans_uses_configured_sprint_length(
        self,
        features: list[Feature],
    ):
        config = DeliveryRiskConfig().model_copy(update={"sprint_length_weeks": 1})

        plans = SprintOverrunService.compute_sprint_plans(features, config=config)

        assert plans["H1: Simplified UI"].planned_sprints == 8
        assert plans["H1: Simplified UI"].sprint_length_weeks == 1
...
class TestSimulateDelivery:
    ...
    def test_cancelled_scenarios_have_zero_business_value(
        ...
        if np.any(cancelled_mask):
            assert np.allclose(result.business_value_after_risk[cancelled_mask], 0.0)
    ...
    def test_actual_cost_uses_configured_sprint_length(self, features: list[Feature]):
        config = DeliveryRiskConfig(
            delay_model=DelayModelConfig(sprint_uncertainty=0.0),
            cancellation=CancellationConfig(
                max_sprints_over_plan=10,
                cancellation_probability=0.0,
            ),
        ).model_copy(update={"sprint_length_weeks": 1})
        feature = features[0]
        plan = SprintPlan(feature_name=feature.name, planned_sprints=4)
        business_value = np.full(500, 80_000.0)
        ...
        expected_weekly_burn = float(feature.development_cost) / float(
            feature.development_weeks
        )
        expected_cost = 4 * expected_weekly_burn * 1
        assert np.allclose(result.actual_cost, expected_cost)
...
class TestComputeProfitability:
    ...
    def test_zero_business_value_means_always_loss(self, features: list[Feature]):
        ...
        result = SprintOverrunService.compute_profitability(delivery, features)
        assert result.portfolio_break_even_probability == pytest.approx(0.0)
...
class TestDeliverySimulationResultProperties:
    def test_cancelled_pct(self):
        ...
        assert result.cancelled_pct == pytest.approx(0.5)
    ...
    def test_sunk_cost_if_cancelled(self):
        ...
        assert result.sunk_cost_if_cancelled == pytest.approx(97_500.0)
    ...
    def test_sunk_cost_zero_when_no_cancellation(self):
        ...
        assert result.sunk_cost_if_cancelled == pytest.approx(0.0)
...
class TestProfitabilityResultProperties:
    def test_break_even_probability(self):
        ...
        assert result.break_even_probability == pytest.approx(0.5)
    ...
    def test_expected_loss(self):
        ...
        assert result.expected_loss == pytest.approx(4_000.0)
...
class TestDeliveryRiskConfig:
    def test_defaults(self):
        ...
        assert config.delay_model.get_uncertainty_decimal() == pytest.approx(0.3)
    ...
    def test_custom_values(self):
        ...
        assert config.delay_model.get_uncertainty_decimal() == pytest.approx(0.4)
...
class TestPlannedSprintsForWeeksValidation:
    def test_raises_on_zero_sprint_length(self):
        with pytest.raises(ValueError, match="sprint_length_weeks must be > 0"):
            SprintOverrunService.planned_sprints_for_weeks(8, sprint_length_weeks=0)
