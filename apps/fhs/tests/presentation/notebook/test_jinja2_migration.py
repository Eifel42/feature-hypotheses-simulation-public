...
class TestOptimizerResultsCardOutput:
    def test_contains_optimizer_data(self):
        ...
        assert "BUDGET USED" in r
        assert "66.7%" in r
        assert "EXPECTED PROFIT" in r
        assert "VAR 95%" in r
        assert "✅" in r

    def test_explicit_budget_pressure_is_kept(self):
        from src.fhs.presentation.notebook.widgets.portfolio import (
            optimizer_results_card,
        )

        r = optimizer_results_card(
            ["A"], 100000, 200000, 80000, 50000, budget_pressure=-0.05
        )
        assert "BUDGET PRESSURE" in r
        assert "-5.0%" in r
