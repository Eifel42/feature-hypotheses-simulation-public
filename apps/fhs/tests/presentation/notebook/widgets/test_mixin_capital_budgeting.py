...
class TestYear1Overview:
    def _make_rows(self, is_total: bool = False) -> tuple:
        return (
...
class TestIrrDualTable:
    def _make_row(self, irr_a: float = 0.15, irr_b: float = 0.22) -> tuple:
        return (
            "Feature A",
            100_000.0,
            20_000.0,
...
class TestCapitalBudgetingSummary:
    def _make_npv(self, expected: float, var_95: float) -> object:
        return SimpleNamespace(expected=expected, var_95=var_95)

    def _make_irr(self, expected: float) -> object:
...
class TestNpvComparisonChart:
    def _make_npv(self, expected: float, var_95: float, p95: float) -> object:
