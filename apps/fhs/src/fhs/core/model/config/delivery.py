...
"""Deployment risk configuration value objects."""
...
class DeliveryRiskConfig(BaseModel):
    """Configuration block for sprint-based deployment risk simulation.
    ...
    scenarios: int = Field(
        ...
        ge=100,
        description=(
            "Number of Monte Carlo scenarios for deployment risk simulation. "
            "Higher values give more precise tail metrics at the cost of runtime. "
            "Overrides the global scenario.scenarios for deployment-specific analysis."
        ),
    )
    sprint_length_weeks: Literal[2] = Field(
        default=2,
        description="Sprint length is fixed to 2 weeks in deployment-risk simulation.",
    )
