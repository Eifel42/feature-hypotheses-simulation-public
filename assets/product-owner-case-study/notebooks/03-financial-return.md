...
PI is the right metric when **budget is scarce**. Two features can both have positive NPV; PI tells you which one creates more value **per euro invested**. The portfolio optimiser in [Notebook 04](04-portfolio-and-budget.md) uses this implicitly when choosing the bundle that maximises total NPV within the budget constraint.
...
NPV and IRR tell you whether a single feature pays back. The next question is **which bundle** to fund within the budget — and whether to keep reserve instead of spending everything: [Notebook 04 — Portfolio And Budget](04-portfolio-and-budget.md).
