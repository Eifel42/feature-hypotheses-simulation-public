...
> The optimiser uses Option A (upfront investment) from [Notebook 03](03-financial-return-evidence.md). All development cost is paid in Year 0; operating costs are deducted from business value each year. ILP (linear) optimisation only — no installment spreading.
...
- **`var_floor`** uses the *simulated* VaR 95% floor from [Notebook 02](02-business-value-evidence.md) minus the upfront cost. This selects the bundle with the strongest downside protection. Use it when you cannot afford a worst-case shortfall.
...
This is the central insight the Overview captures: **the best decision is not always to spend the full budget.** Budget discipline means spending where the roadmap gets the best evidence-adjusted result, not maximising spend.
...
- **Upfront investment model.** Development cost is paid at Year 0; operating costs accrue each year. Installment financing is a separate analysis in [Notebook 03](03-financial-return-evidence.md).
...
The reserve has a concrete purpose: it stays available for the next decision gate. If [Notebook 05](05-risk-resilience-evidence.md) or [Notebook 06](06-development-cost-risk-evidence.md) surfaces a tail risk that needs mitigation budget, the reserve covers it. If H1's value case strengthens later (better growth data, lower development risk), the reserve can fund a re-evaluated H1.
...
| **Upfront investment** | All development cost paid in Year 0 (Option A from [Notebook 03](03-financial-return-evidence.md)). No installment spreading. |
...
The portfolio choice (H2 + H3) is the input to the risk-resilience stress test in [Notebook 05 — Risk Resilience Evidence](05-risk-resilience-evidence.md), where the bundle is subjected to development, market, component, and global shocks.
