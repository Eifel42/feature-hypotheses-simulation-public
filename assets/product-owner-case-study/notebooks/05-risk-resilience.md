...
Each scenario draws a random number for each feature. If the draw falls below the feature's LLP, the feature is not completed and produces zero business value:
...
After all four risk gates are applied, the same metrics from [Notebook 02](02-business-value.md) are recomputed on the **risk-adjusted** distribution:
...
The portfolio selection from [Notebook 04](04-portfolio-and-budget.md) used `var_floor` as the ranking strategy. Notebook 05 also compares it with three alternatives:
...
When the top ranks differ across strategies, the decision **depends strongly on the chosen risk perspective**. The recommendation in this case study is robust because H2 and H3 dominate the top under multiple strategies — but a single strategy is not enough decision basis for a strong claim.
...
Risk Resilience showed *that* development dominates the downside. The next question is *how much* development cost overruns can add to the bill, and which cancellation rule applies if a feature runs late: [Notebook 06 — Development Cost Risk](06-development-cost-risk.md).
