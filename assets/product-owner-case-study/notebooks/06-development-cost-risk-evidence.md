...
|---|---:|---:|---:|---:|---:|
| H1 Simplified UI | 75,000 EUR | 100,762 EUR | 150,000 EUR | 150,000 EUR | +100% |
| H2 Traceability | 55,000 EUR | 73,894 EUR | 110,000 EUR | 110,000 EUR | +100% |
| H3 Expiration Alerts | 10,509 EUR | 16,887 EUR | 21,018 EUR | 21,018 EUR | +100% |

These are **CVaR outputs**, not a separate planning estimate. In this specific configuration, individual CVaR 95% equals individual LaR / CaR 95% because the worst 5% of simulated sprint outcomes are capped by `sprint_ceiling: 1.5`. The metric remains CVaR: if the ceiling or delay distribution changes, CVaR can separate from the percentile boundary again.

For the selected H2 + H3 portfolio, the portfolio CVaR is lower than the sum of the individual CVaRs because the calculation uses the per-scenario summed cost distribution:
...
The conservative sum of individual CVaRs would be `110,000 + 21,018 = 131,018 EUR`, but that is **not** the Notebook 06 portfolio metric. The Notebook 06 metric is `126,938 / 65,509 - 1 = 93.8%`.
