...
Will this feature stay within budget if the team runs late? Notebook 06 is **pure development cost and schedule risk** — business value is not modelled here (it lives in [Notebook 02](02-business-value.md)) and risk to value lives in [Notebook 05](05-risk-resilience.md). The output of this notebook is one set of numbers:
...
### Why Lognormal — Empirical Findings
...
- If the check fires, cancellation occurs with probability `cancellation_probability` (configured at 60% in this case study).
- When a feature is cancelled, **business value is zero** — the feature is not completed.
...
For the case study (`max_sprints_over_plan` = 2, `cancellation_probability` = 60%):

| Feature | `S_plan` | Cancellation trigger sprint | Sprint ceiling (2.5x `S_plan`, rounded up) |
|---|---:|---:|---:|
| H1 | 3 | 5 | 8 |
| H2 | 3 | 5 | 8 |
| H3 | 2 | 4 | 5 |

With the 2.5× sprint ceiling, the cancellation trigger sits inside the simulated distribution for every feature: H1 and H2 have three sprints of headroom above the trigger (5 → 8), H3 has one sprint (4 → 5). The cancellation rule therefore fires with realistic frequency — roughly 1.7 % of scenarios for H1/H2 and 0.3 % for H3. H3's rate is low not because it is safe, but because its P95 sits exactly at the trigger.
...
The Overview lists the configured parameters in one table. All values come from the development-risk configuration block in [blockchain.yaml](../../../apps/fhs/notebooks/config/blockchain.yaml) — the notebook reads them directly; nothing is hard-coded.
...
| Cancellation threshold | `cancellation.max_sprints_over_plan` / `cancellation_probability` | 2 / 0.6 | At more than two sprints over plan there is a 60% chance the feature is cancelled |
...
- **Fits:** the team can develop everything within the quarter on plan.
...
The chart shows the full sprint-count distribution per feature — how spread out development estimates are around the plan. With the 2.5× ceiling, H1 and H2 are capped at 8 actual sprints, while H3 is capped at 5 actual sprints — leaving meaningful room above each feature's cancellation trigger.
...
Notebook 06 now shows the same four cost measures in the chart that the table reports:

![Development cost comparison](../images/06-development-risk-fig2.png)

The expected cost bar is the planning reserve signal. CaR 95% is the steering guardrail, and CVaR 95% is the funding-gate signal for the worst development tail.

| Bar | Product Owner reading |
|---|---|
| **Planned cost** | The approved development budget for the feature. |
| **Expected cost** | The average simulated cost; use it for release reserve planning. |
| **CaR 95%** | The upper steering guardrail; only 5% of scenarios exceed this value. |
| **CVaR 95%** | The average cost in the worst 5% of scenarios; use it for funding-gate judgement. |

This makes the chart proportional to the table. If the CaR and CVaR bars sit close together, the tail is capped tightly. If CVaR rises clearly above CaR, the worst scenarios continue to get more expensive after the 95% boundary.
...
Important boundary: Notebook 06 is a **development-cost risk view**, not the final roadmap optimiser. A feature can fit the development budget and still be rejected by the business-value and financial-return gates. The final Notebook 07 decision selects **2 of 3 features: H2 Traceability + H3 Expiration Alerts**. H1 stays on hold.
...
For the case study, the **relative budget pressure splits cleanly**: H1 and H2 land at roughly +104 %, while H3 jumps to +169 % because one extra sprint nearly doubles its planned cost. H2 still dominates absolute EUR exposure, but H3 has the steepest *relative* tail — exactly what the normalised metric is built to expose.

The individual feature values are:

| Feature | Planned cost | Expected development cost | LaR / CaR 95% | CVaR 95% | Feature budget pressure |
|---|---:|---:|---:|---:|---:|
...
These are **CVaR outputs**, not a separate planning estimate. CVaR 95% sits slightly above CaR 95% for every feature: the worst tail of the distribution has room above the 95th percentile, so the average of that tail (CVaR) is strictly larger than the boundary (CaR). The narrower the gap, the closer the tail is to a hard ceiling.

For the selected H2 + H3 portfolio, the portfolio CVaR uses the per-scenario summed cost distribution — not the sum of the two individual CVaRs:
...
The conservative sum of individual CVaRs would be `112,437 + 28,272 = 140,709 EUR`, but that is **not** the Notebook 06 portfolio metric. The Notebook 06 metric is `136,047 / 65,509 - 1 = 107.7%`.
...
- **A low planned investment does not mean low development risk.** H3 is the cheapest feature on paper, but the simulation puts its budget pressure at +169 % — one extra sprint nearly doubles its bill, and its P95 already sits at the cancellation trigger.
...
Combined with the portfolio choice from [Notebook 04](04-portfolio-and-budget.md) (H2 + H3), the implication is direct:

- Hold **H2 inside one release window** because its absolute cost dominates the development risk.
- Apply a **tight scope gate to H3** because its relative cost range is widest (+169 % vs. ~104 % for H1/H2) — small overruns produce large proportional damage.

Three Scrum-language uses:

| Backlog question | What this notebook gives you |
|---|---|
| Why reduce H3 scope before sprint commitment? | Highest relative budget pressure (+169 %); a single extra sprint nearly doubles the planned cost. |
...
Development cost is the last decision-basis layer. The synthesis — turning all five dimensions (Business Value, Financial Return, Portfolio Choice, Risk Resilience, Development Cost Risk) into one executive recommendation with a traffic-light verdict — is [Notebook 07 — Executive Decision](07-executive-decision.md).
