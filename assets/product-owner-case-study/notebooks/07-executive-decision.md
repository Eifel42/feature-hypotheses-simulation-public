...
<< [Previous: Notebook 06 - Development Cost Risk](06-development-cost-risk.md) | [Case Study Overview](../product-owner-case-study.md)

---

This guide transfers Notebook 07 - the synthesis. It explains how the five decision-basis layers from Notebooks 02-06 collapse into one traffic-light verdict, why the CONDITIONAL GO recommendation is defensible, what the robustness check tells you, and what to do depending on the verdict.

## The Question

One notebook. All dimensions. One recommendation. Five separate decision-basis layers (Business Value, Financial Return, Portfolio Optimisation, Risk Profile, Development Cost Risk) get reduced to a single GO / CONDITIONAL GO / REVIEW signal and a concrete action playbook.

The value of Notebook 07 is **not** another calculation. Its value is **traceability**: every recommendation can be walked back to one earlier decision-basis layer.
...
|:-:|---|---|---|
| 1 | Business Value | [Notebook 02](02-business-value.md) | What value can each feature provide? |
| 2 | Financial Return | [Notebook 03](03-financial-return.md) | Is the investment worthwhile? (NPV, IRR) |
| 3 | Portfolio Optimisation | [Notebook 04](04-portfolio-and-budget.md) | Which features should we build at this budget? |
| 4 | Risk Profile | [Notebook 05](05-risk-resilience.md) | What can go wrong and how much value is at risk? |
| 5 | Development Cost Risk | [Notebook 06](06-development-cost-risk.md) | Will development stay within budget if sprints run late? |
...
| Development Cost Risk | H2 needs release-window control; H3 needs an early scope gate — its relative budget pressure is highest (+169 %) because one extra sprint nearly doubles its planned cost. | CONDITIONAL GO |
...
> We should not fund the full roadmap now. The decision basis supports H2 and a controlled H3 test. H1 should wait. The main risk is not market demand; it is development confidence. We should keep reserve and pass the next gate only when development risk improves.
...
1. Confirm financing option - use installment (Option B from [NB 03](03-financial-return.md)) to reduce Year-1 cash outflow.
...
Phasing turns CONDITIONAL GO into a **decision rhythm**, not a single yes/no. The reserve is what makes the rhythm possible - without it, the next decision is constrained before new findings arrive.
...
- **CONDITIONAL GO is not weak.** It is a defensible "proceed with controls" - far better than "yes" without a decision basis or "no" without analysis.
...
The earlier notebooks build the decision basis. This notebook turns that decision basis into an executive roadmap decision.
...
| **Five dimensions** | Business Value, Financial Return, Portfolio Optimisation, Risk Profile, Development Cost Risk. |
...
| **Phase rollout** | Treating CONDITIONAL GO as a decision rhythm - fund Phase 1, re-evaluate at the next gate, scale or cancel based on new findings. |
| **Traceability** | Each recommendation can be walked back to one source notebook and one piece of the decision basis. The point of Notebook 07. |

## End Of The Reading Path

This is the last notebook in the case-study sequence. The role-specific reading and the executive verdict are now complete.
...
|:-:|---|---|
| 02 | [Business Value](02-business-value.md) | What value can each feature provide? |
| 03 | [Financial Return](03-financial-return.md) | Is the investment worthwhile? (NPV, IRR) |
| 04 | [Portfolio Optimisation](04-portfolio-and-budget.md) | Which features should we build at this budget? |
| 05 | [Risk Profile](05-risk-resilience.md) | What can go wrong and how much value is at risk? |
| 06 | [Development Cost Risk](06-development-cost-risk.md) | Will development stay within budget if sprints run late? |
...
For deeper portfolio analysis (10-feature portfolios, three solvers, concentration risk), see the advanced notebooks:
