...
The decision must replace opinions with a simple risk/return view that shows what each feature delivers and how low the floor can go.
...
- For the Product Owner: how much business value is at risk if this feature is not delivered.
...
- A **green +** means the feature covers its costs in Year 1. A **red −** means it does not — which may still be acceptable over a multi-year horizon (see [Notebook 03](03-financial-return-evidence.md)).
...
Three Scrum-language uses for the same evidence:
...
Once business value is sized, the next question is whether the investment pays back over time after discounting and financing decisions: [Notebook 03 — Financial Return Evidence](03-financial-return-evidence.md).
